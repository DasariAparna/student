...
mydb = mysql.connector.connect(host='localhost',user='root',password='appu',database='user1')
...
def register():
    if request.method == 'POST':
        ...
        cursor.execute('select count(*) from user1 where email=%s',[Mail])
...
def confirm(token):
    try :
        ...
        user1info = datta.loads(token,salt=salt1,max_age=200)
    except Exception as e:
        #print(e)
        return 'sorry this link expiredddd plz register after somettime Have a Good Day'
    else:
        cursor = mydb.cursor(buffered=True)
        cursor.execute('insert into user1(Name,Email,pwd,phno,place) values(%s,%s,%s,%s,%s)',[user1info['name'],user1info['mail'],user1info['pwd'],user1info['ph'],user1info['place']])
        mydb.commit()
        cursor.close()
...
def login():
    ...
    if request.method == 'POST':
        ...
        cursor = mydb.cursor(buffered=True)
        cursor.execute('select Email,pwd from user1 where Email=%s',[email])
        e_mail,password = cursor.fetchone()
        cursor.execute('select Name from user1 where Email=%s',[email])
...
def forgot(token):
    try :
        ...
        users = ram.loads(token,salt=salt2,max_age=200)
    except Exception as e:
        print(e)
        ...
        if request.method == 'POST':
            ...
            if newpwd == confirmpwd:
                cursor = mydb.cursor(buffered=True)
                cursor.execute('update user1 set pwd=%s where Email=%s',[newpwd,users])
                mydb.commit()
                cursor.close()
...
def addnotes():
    if request.method =='POST':
        ...
        cursor = mydb.cursor()
        cursor.execute('insert into notes(title,content,added_by) values(%s,%s,%s)',[a,b,w])
        mydb.commit()
        cursor.close()
        flash('ur notes created successfully......!') 
        return redirect(url_for('Dashboard'))
